...
export async function fetchAllKelas({ params }: { params: FilterkelasDto }) {
  ...
  const req = await api.get(`/kelas?${url.toString()}`)
  return req.data
}
...
// UPDATE
export async function updateKelas({ id, data }: { id: string; data: UpsertKelasDto }) {
...
export function useUpdateKelas() {
  ...
  return useMutation({
    ...
    onSuccess: (data, variables) => {
      queryClient.invalidateQueries({ queryKey: KelasKeys.detail(variables.id) })
      queryClient.invalidateQueries({ queryKey: KelasKeys.lists() })
      toast("Mata pelajaran berhasil diupdate")
    },
    onError: (error: any) => {
      toast(error?.response?.data?.message || "Gagal mengupdate mata pelajaran")
    },
  })
}
  ...
  return useMutation({
    ...
    onError: (err, variables, context) => {
      if (context?.previousData) {
        queryClient.setQueryData(KelasKeys.detail(context.id), context.previousData)
      }
